from flask import (abort, jsonify, request, render_template, request, url_for, redirect, session, )
...
def register():
    ...
    if request.method == "GET":
        ...
        if User.query.filter(User.email == request.form.get('email')).first():
            abort(409,description=f'User with username {request.form.get("email")} already exists')
...
def logout():
    ...
    if crn_user:
        ...
        logout_user()
        return (jsonify({"status": 200, "current_user": f"User {str(crn_user_name)} logout"}), 200,)
    else:
        return (jsonify({"status": 404, "message": "Any user available at this session"}), 404,)
